...
-- Script date 13.10.2024 1:22:03
...
--
-- Drop procedure `delete_experiences_skills`
--
DROP PROCEDURE IF EXISTS delete_experiences_skills;
...
--
-- Drop procedure `delete_skills`
--
DROP PROCEDURE IF EXISTS delete_skills;

--
-- Drop procedure `delete_template_all`
--
DROP PROCEDURE IF EXISTS delete_template_all;
...
--
-- Drop procedure `insert_skill_show`
--
DROP PROCEDURE IF EXISTS insert_skill_show;
...
--
-- Drop procedure `update_experience`
--
...
--
-- Drop procedure `insert_experience`
--
...
--
-- Drop procedure `update_resume`
--
DROP PROCEDURE IF EXISTS update_resume;
...
--
-- Drop procedure `update_filepathes`
--
DROP PROCEDURE IF EXISTS update_filepathes;
...
CREATE TABLE resumes (
  ...
  resume_introduction text DEFAULT NULL,
  cv_introduction text DEFAULT NULL,
  resume_doc_filepath varchar(510) DEFAULT NULL,
  cv_doc_filepath varchar(510) DEFAULT NULL,
  cl_doc_filepath varchar(510) DEFAULT NULL,
  resume_pdf_filepath varchar(510) DEFAULT NULL,
  cv_pdf_filepath varchar(510) DEFAULT NULL,
  cl_pdf_filepath varchar(510) DEFAULT NULL,
...
--
-- Create procedure `update_filepathes`
--
CREATE PROCEDURE update_filepathes (IN p_name varchar(50), IN p_region_id varchar(2), IN p_resume_doc_filepath varchar(510), IN p_cv_doc_filepath varchar(510), IN p_cl_doc_filepath varchar(510), IN p_resume_pdf_filepath varchar(510), IN p_cv_pdf_filepath varchar(510), IN p_cl_pdf_filepath varchar(510))
SQL SECURITY INVOKER
BEGIN
  IF (p_resume_doc_filepath IS NOT NULL) THEN
    UPDATE resumes
    SET resume_doc_filepath = p_resume_doc_filepath
    WHERE name = p_name
    AND region_id = p_region_id;
  END IF;

  IF (p_cv_doc_filepath IS NOT NULL) THEN
    UPDATE resumes
    SET cv_doc_filepath = p_cv_doc_filepath
    WHERE name = p_name
    AND region_id = p_region_id;
  END IF;

  IF (p_cl_doc_filepath IS NOT NULL) THEN
    UPDATE resumes
    SET cl_doc_filepath = p_cl_doc_filepath
    WHERE name = p_name
    AND region_id = p_region_id;
  END IF;

  IF (p_resume_pdf_filepath IS NOT NULL) THEN
    UPDATE resumes
    SET resume_pdf_filepath = p_resume_pdf_filepath
    WHERE name = p_name
    AND region_id = p_region_id;
  END IF;

  IF (p_cv_pdf_filepath IS NOT NULL) THEN
    UPDATE resumes
    SET cv_pdf_filepath = p_cv_pdf_filepath
    WHERE name = p_name
    AND region_id = p_region_id;
  END IF;
  ...
  IF (p_cv_pdf_filepath IS NOT NULL) THEN
    UPDATE resumes
    SET cl_pdf_filepath = p_cv_pdf_filepath
    WHERE name = p_name
    AND region_id = p_region_id;
  END IF;
...
CREATE TABLE resume_footers (
  ...
  cv_include_footer tinyint(1) NOT NULL DEFAULT 1,
  created timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  updated timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
...
--
-- Create index `UK_resume_footers` on table `resume_footers`
--
ALTER TABLE resume_footers
ADD UNIQUE INDEX UK_resume_footers (resume_id, footer_order);
...
--
CREATE PROCEDURE update_resume (IN p_resume_id integer, IN p_name varchar(50), IN p_job_opportunity varchar(255), IN p_job_place varchar(255), IN p_phone_numbers_text varchar(512), IN p_resume_introduction text, IN p_cv_introduction text, IN p_archived integer, IN p_footer_1_header varchar(255), IN p_footer_1_text text, IN p_footer_2_header varchar(255), IN p_footer_2_text text, IN p_footer_3_header varchar(255), IN p_footer_3_text text, IN p_footer_4_header varchar(255), IN p_footer_4_text text, IN p_cv_include_footer1 boolean, IN p_cv_include_footer2 boolean, IN p_cv_include_footer3 boolean, IN p_cv_include_footer4 boolean, OUT p_result integer)
SQL SECURITY INVOKER
BEGIN
  SET p_result = 1;
  ...
  SET name = p_name,
      ...
      cv_introduction = p_cv_introduction,
      archived = p_archived
  ...
  IF LENGTH(TRIM(p_footer_1_header)) + LENGTH(TRIM(p_footer_1_text)) > 0 THEN
    INSERT INTO resume_footers (resume_id, footer_header, footer_text, footer_order, cv_include_footer)
      VALUES (p_resume_id, p_footer_1_header, p_footer_1_text, 1, IFNULL(p_cv_include_footer1, FALSE));
  END IF;
  IF LENGTH(TRIM(p_footer_2_header)) + LENGTH(TRIM(p_footer_2_text)) > 0 THEN
    INSERT INTO resume_footers (resume_id, footer_header, footer_text, footer_order, cv_include_footer)
      VALUES (p_resume_id, p_footer_2_header, p_footer_2_text, 2, IFNULL(p_cv_include_footer2, FALSE));
  END IF;

  IF LENGTH(TRIM(p_footer_3_header)) + LENGTH(TRIM(p_footer_3_text)) > 0 THEN
    INSERT INTO resume_footers (resume_id, footer_header, footer_text, footer_order, cv_include_footer)
      VALUES (p_resume_id, p_footer_3_header, p_footer_3_text, 3, IFNULL(p_cv_include_footer3, FALSE));
  END IF;

  IF LENGTH(TRIM(p_footer_4_header)) + LENGTH(TRIM(p_footer_4_text)) > 0 THEN
    INSERT INTO resume_footers (resume_id, footer_header, footer_text, footer_order, cv_include_footer)
      VALUES (p_resume_id, p_footer_4_header, p_footer_4_text, 4, IFNULL(p_cv_include_footer4, FALSE));
  END IF;
  SET p_result = 0;
END
$$
...
--
CREATE PROCEDURE archive_resume_all (IN resume_id int)
SQL SECURITY INVOKER
BEGIN
...
CREATE TABLE templates (
  ...
  job_place varchar(255) DEFAULT 'Location: remote job',
  phone_numbers_text varchar(255) DEFAULT '+380931775176 (Viber, Whatsapp)',
  template_introduction text DEFAULT NULL,
...
--
CREATE PROCEDURE update_template (IN p_template_id integer, IN p_name varchar(50), IN p_job_opportunity varchar(255), IN p_job_place varchar(255), IN p_phone_numbers_text varchar(255), IN p_template_introduction text, IN p_archived integer, IN p_footer_1_header varchar(255), IN p_footer_1_text varchar(2000), IN p_footer_2_header varchar(255), IN p_footer_2_text varchar(2000), IN p_footer_3_header varchar(255), IN p_footer_3_text varchar(2000), IN p_footer_4_header varchar(255), IN p_footer_4_text varchar(2000), OUT p_result integer)
SQL SECURITY INVOKER
BEGIN
  SET p_result = 1;
...
--
CREATE PROCEDURE archive_templates_all (IN p_template_id int)
SQL SECURITY INVOKER
BEGIN
...
ADD CONSTRAINT CK_experiences2 CHECK ((`resume_id` IS NULL) OR (`template_id` IS NULL));
...
--
CREATE PROCEDURE insert_experience (IN p_flag varchar(20), IN p_resume_id integer, IN p_template_id integer, IN p_job_order integer, IN p_job_position varchar(255), IN p_start_date datetime, IN p_end_date datetime, IN p_employer varchar(255), IN p_resp text, IN p_benefits varchar(1000), IN p_other text, OUT p_experience_id int)
SQL SECURITY INVOKER
BEGIN
  IF (p_flag = 'resume_id') THEN
    INSERT INTO experiences (resume_id, template_id, job_order, job_position, start_date, end_date, employer, responsibilities, benefits, other)
      VALUES (p_resume_id, NULL, p_job_order, p_job_position, p_start_date, p_end_date, p_employer, p_resp, p_benefits, p_other);
    SET p_experience_id = LAST_INSERT_ID();
  END IF;

  IF (p_flag = 'template_id') THEN
    INSERT INTO experiences (resume_id, template_id, job_order, job_position, start_date, end_date, employer, responsibilities, benefits, other)
      VALUES (NULL, p_template_id, p_job_order, p_job_position, p_start_date, p_end_date, p_employer, p_resp, p_benefits, p_other);
    SET p_experience_id = LAST_INSERT_ID();
  END IF;
...
--
CREATE PROCEDURE insert_skill (IN p_skill varchar(255), IN p_category varchar(255), OUT p_skill_id int)
SQL SECURITY INVOKER
BEGIN
  DECLARE l_skill_id integer;
  DECLARE l_category_id integer;

  SELECT
    MAX(id) INTO l_category_id
  FROM skills_categories
  WHERE LOWER(category) = LOWER(p_category);
  IF (p_category = '')
    OR (l_category_id IS NULL) THEN
    SET l_category_id = 0;
  END IF;
...
--
ALTER TABLE skill_show_lists
ADD CONSTRAINT FK_skill_show_lists_skill_id FOREIGN KEY (skill_id)
...
--
-- Create procedure `update_experience`
--
CREATE PROCEDURE update_experience (IN p_flag varchar(20), IN p_resume_id integer, IN p_template_id integer, IN p_job_order integer, IN p_job_position varchar(255), IN p_start_date datetime, IN p_end_date datetime, IN p_employer varchar(255), IN p_resp text, IN p_benefits varchar(1000), IN p_other text, OUT p_experience_id int)
SQL SECURITY INVOKER
BEGIN
  IF (p_flag = 'resume_id') THEN
    DELETE
...
--
CREATE PROCEDURE insert_template (IN p_name varchar(50), IN p_job_opportunity varchar(255), IN p_job_place varchar(255), IN p_phone_numbers_text varchar(255), IN p_template_introduction text, IN p_footer_1_header varchar(255), IN p_footer_1_text varchar(2000), IN p_footer_2_header varchar(255), IN p_footer_2_text varchar(2000), IN p_footer_3_header varchar(255), IN p_footer_3_text varchar(2000), IN p_footer_4_header varchar(255), IN p_footer_4_text varchar(2000), OUT p_template_id integer, OUT p_result integer)
SQL SECURITY INVOKER
BEGIN
  SET p_result = 1;
  SELECT
...
--
-- Create procedure `insert_skill_show`
--
CREATE PROCEDURE insert_skill_show (IN p_experience_id int, IN p_skill varchar(255))
SQL SECURITY INVOKER
BEGIN
  DECLARE l_skill_id integer;
  SELECT
...
--
CREATE PROCEDURE insert_resume (IN p_name varchar(50), IN p_lang varchar(3), IN p_region_id varchar(2), IN p_job_opportunity varchar(255), IN p_job_place varchar(255), IN p_phone_numbers_text varchar(255), IN p_resume_introduction text, IN p_footer_1_header varchar(255), IN p_footer_1_text text, IN p_footer_2_header varchar(255), IN p_footer_2_text text, IN p_footer_3_header varchar(255), IN p_footer_3_text text, IN p_footer_4_header varchar(255), IN p_footer_4_text text, IN p_archived tinyint, IN p_cv_introduction text, IN p_cv_include_footer1 boolean, IN p_cv_include_footer2 boolean, IN p_cv_include_footer3 boolean, IN p_cv_include_footer4 boolean, OUT p_resume_id integer)
SQL SECURITY INVOKER
BEGIN
  SELECT
  ...
  DELETE
    FROM resumes
  WHERE id = p_resume_id;
  INSERT INTO resumes (name, lang, region_id, job_opportunity, job_place, phone_numbers_text, resume_introduction, cv_introduction, archived)
    VALUES (p_name, p_lang, p_region_id, p_job_opportunity, p_job_place, p_phone_numbers_text, p_resume_introduction, p_cv_introduction, p_archived);
  SELECT
    LAST_INSERT_ID() INTO p_resume_id;
  IF NOT (IFNULL(p_footer_1_header, '') = ''
    AND IFNULL(p_footer_1_text, '') = '') THEN
    INSERT INTO resume_footers (resume_id, footer_header, footer_text, footer_order, cv_include_footer)
      VALUES (p_resume_id, p_footer_1_header, p_footer_1_text, 1, p_cv_include_footer1);
  END IF;
  IF NOT (IFNULL(p_footer_2_header, '') = ''
    AND IFNULL(p_footer_2_text, '') = '') THEN
    INSERT INTO resume_footers (resume_id, footer_header, footer_text, footer_order, cv_include_footer)
      VALUES (p_resume_id, p_footer_2_header, p_footer_2_text, 2, p_cv_include_footer2);
  END IF;
  IF NOT (IFNULL(p_footer_3_header, '') = ''
    AND IFNULL(p_footer_3_text, '') = '') THEN
    INSERT INTO resume_footers (resume_id, footer_header, footer_text, footer_order, cv_include_footer)
      VALUES (p_resume_id, p_footer_3_header, p_footer_3_text, 3, p_cv_include_footer3);
  END IF;
  IF NOT (IFNULL(p_footer_4_header, '') = ''
    AND IFNULL(p_footer_4_text, '') = '') THEN
    INSERT INTO resume_footers (resume_id, footer_header, footer_text, footer_order, cv_include_footer)
      VALUES (p_resume_id, p_footer_4_header, p_footer_4_text, 4, p_cv_include_footer4);
  END IF;
END
$$

--
-- Create procedure `delete_template_all`
--
CREATE PROCEDURE delete_template_all (IN p_template_id int)
SQL SECURITY INVOKER
BEGIN
  DELETE
    FROM skill_show_lists l
...
--
CREATE PROCEDURE delete_skills (IN p_skill_id int)
SQL SECURITY INVOKER
BEGIN
  DELETE
    FROM skill_show_lists
...
--
CREATE PROCEDURE delete_resume_all (IN p_resume_id int)
SQL SECURITY INVOKER
BEGIN
  DELETE
    FROM skill_show_lists l
...
--
-- Create procedure `delete_experiences_skills`
--
CREATE PROCEDURE delete_experiences_skills (IN p_flag varchar(50), IN p_template_id int, IN p_resume_id int)
SQL SECURITY INVOKER
BEGIN
  IF (p_flag = 'resume_id') THEN
    DELETE
...
--
CREATE PROCEDURE archive_template_id (IN p_template_id int, OUT p_result int)
SQL SECURITY INVOKER
BEGIN
  SET p_result = 1;
  DELETE
...
CREATE TABLE positions (
  ...
  archived tinyint NOT NULL DEFAULT 0 COMMENT 'Отправить в архив',
...
CREATE TABLE emails (
  ...
  mailbox_received TINYINT DEFAULT NULL,
  db_received TINYINT DEFAULT NULL,
  created TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  updated TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id)
)
...
CREATE TABLE activity_tasks (
  ...
  sheduler_id int DEFAULT NULL,
  activity_description varchar(1000) DEFAULT NULL,
  archived tinyint DEFAULT NULL
)
ENGINE = INNODB,
CHARACTER SET utf8mb4,
...
('DE', 'Germany', 'yyyy-mm-dd', 40, '2023-12-28 15:25:18', '2024-02-22 22:44:42'),
('EN', 'England', 'dd/mm/yyyy', 50, '2023-12-28 15:25:29', '2024-01-02 09:14:45'),
('HR', 'Croatia', 'dd-mm-yyyy', 30, '2023-12-28 15:25:04', '2024-01-02 09:14:39'),
('PO', 'Poland', 'dd.mm.yyyy', 20, '2023-12-28 15:24:53', '2024-01-02 09:14:36'),
('RU', 'Template', 'dd.mm.yyyy', 0, '2024-01-27 18:06:32', '2024-01-27 18:06:32'),
('UA', 'Ukraine', 'dd.mm.yyyy', 10, '2023-12-28 15:24:29', '2024-01-02 09:29:11'),
('US', 'USA-Canada', 'mm-dd-yyyy', 60, '2023-12-28 15:25:40', '2024-03-11 23:08:25');

-- 
-- Dumping data for table languages
--
INSERT INTO languages VALUES
('DE', 'Немецкий', 40, '2023-12-28 15:51:33', '2023-12-28 16:32:31'),
('EN', 'Английский', 30, '2023-12-28 15:51:22', '2023-12-28 16:32:29'),
('HR', 'Хорватский', 50, '2023-12-28 15:52:26', '2023-12-28 16:32:38'),
('PL', 'Польский', 60, '2023-12-28 15:52:35', '2023-12-28 16:32:41'),
('RU', 'Русский', 10, '2023-12-28 15:50:47', '2023-12-28 16:32:23'),
('UA', 'Украинский', 20, '2023-12-28 15:51:06', '2023-12-28 16:32:26');

-- 
-- Dumping data for table skills_categories
--
INSERT INTO skills_categories VALUES
(0, 'NULL', '2024-02-28 23:01:18', '2024-02-28 23:01:36'),
(1, 'Database', '2024-01-06 21:10:39', '2024-02-29 02:20:03'),
(2, 'Support', '2024-01-06 21:10:48', '2024-02-29 02:20:06'),
(3, 'Software', '2024-02-28 22:53:03', '2024-02-29 02:20:08'),
(4, 'Project management', '2024-02-28 22:53:33', '2024-02-29 02:20:11'),
(6, 'Clouds', '2024-02-29 02:16:25', '2024-02-29 02:20:13'),
(7, 'ETL', '2024-02-29 02:17:10', '2024-02-29 02:17:10'),
(8, 'Reports', '2024-02-29 02:18:08', '2024-02-29 02:18:08');

-- 
-- Dumping data for table resumes
--
INSERT INTO resumes VALUES
(82, 'Oracle-Entwickler', 'DE', 'DE', 'Oracle-Entwickler', 'Fernarbeit', '+(385) 998 511 378', 'dfdfd df dfd fdf df df df df df dfdfd df dfd fdf df df df df df dfdfd df dfd fdf df df df df df dfdfd df dfd fdf df df df df df dfdfd df dfd fdf df df df df df dfdfd df dfd fdf df df df df df dfdfd df dfd fdf df df df df df dfdfd df dfd fdf df df df df df dfdfd df Recommendation letter: \r\n\r\n\r\nhttps://t.me/Data_migration_club\r\nRecommendation letter: https://bit.ly/3ITcypl', '', 'd:\\Мой диск\\Поиск работы\\Germany\\r\\r-oracle-entwickler-de.docx', 'd:\\Мой диск\\Поиск работы\\Germany\\cv\\oracle-entwickler-de.docx', 'd:\\Мой диск\\Поиск работы\\Germany\\cl\\oracle-entwickler-de.docx', 'd:\\Мой диск\\Поиск работы\\Germany\\r\\oracle-entwickler-de.pdf', 'd:\\Мой диск\\Поиск работы\\Germany\\cv\\oracle-entwickler-de.pdf', 'd:\\Мой диск\\Поиск работы\\Germany\\cv\\oracle-entwickler-de.pdf', NULL, NULL, 0, '2024-03-06 22:50:59', '2024-10-12 18:56:41'),
(85, 'Database developer', 'UA', 'UA', 'Database developer', 'Віддалена робота', '+(380) 93 177 51 76 Viber, Whatsapp', 'Зверніть будь ласка увагу на мій загальний досвід 10 років у розробці, впровадженні та технічну підтримку у фінансовій, страховій та банківській сферах. За цей час я отримав експертні знання у галузі проектування баз даних, розробки та оптимізації. Найбільше я працював із реляційними базами даних – Oracle, Microsoft SQL server, Mysql.\r\nЯ працював на технічній підтримці продуктів JasperSoft, програм JasperReports та Jaspesoft ETL і навчився створювати звіти на основі вимог користувачів. До моїх обов''язків входило усунення проблем, пов''язаних зі звітами, та своєчасна відповідь на звернення користувачів. У багатьох проектах команда розробки знаходиться далеко від інших команд, іноді на іншому континенті. То ж мені доводилося співпрацювати з людьми з різних країн світу для підтримки прийнятих рішень. Під час роботи ми дотримувалися встановлених стандартів та рекомендацій щодо організації технічної підтримки згідно ITIL.\r\nЯ працював як розробник баз даних на інших проектах, займався ETL процесами міграції даних. Після фази проектування наша команда займалася створенням потоків даних, автоматичним перенесенням даних із промислових OLTP систем до сховищ (DWH).\r\nЯ закінчив Харківський державний університет, фізичний факультет. Навчання фізикою та математикою допомогли мені розвинути сильні аналітичні здібності та вміння помічати деталі, концентруючись на отриманні зрозумілих результатів.\r\nПрацюючи як програміст більше 10 років, я підвищив рівень знання англійської мови до рівня B2, маю підтвердження цієї оцінки. English Benchmark test https://www.data-migration.club/r/test.jpg \r\nЯ часто буваю онлайн, і віддаю перевагу зустрічам з веб-камерою, живим спілкуванням.\r\nДуже прошу Вас надсилати запрошення електронною поштою. Для швидкого спілкування та коротких питань телефонуйте 093 177 5176.\r\n', 'Зверніть будь ласка увагу на мій загальний досвід 10 років у розробці, впровадженні та технічну підтримку у фінансовій, страховій та банківській сферах. За цей час я отримав експертні знання у галузі проектування баз даних, розробки та оптимізації. Найбільше я працював із реляційними базами даних – Oracle, Microsoft SQL server, Mysql.\r\nEnglish Benchmark test https://www.data-migration.club/r/test.jpg \r\nЯ часто буваю онлайн, і віддаю перевагу зустрічам з веб-камерою, живим спілкуванням.\r\nНадсилайте запрошення на vitaly.makhaev@gmail.com, телефон +(380) 93 177 5176.\r\n', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\r-database-developer-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\database-developer-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cl\\database-developer-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\database-developer-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\database-developer-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\database-developer-ua.pdf', NULL, NULL, 0, '2024-03-20 12:42:43', '2024-10-12 18:57:58'),
(86, 'DB-DBA', 'UA', 'UA', 'DB developer & DBA', 'Віддалена', '+(380) 93 177 51 76 Viber, Whatsapp', 'Зверніть увагу на мій загальний досвід 10+ років у розробці, впровадженні та технічну підтримку програмного забезпечення. Я розробляв та супроводжував програми у торговельній та банківській сферах. За цей час я отримав експертні знання у галузі проектування баз даних, розробки та оптимізації. Найбільше я працював із реляційними базами даних – Microsoft SQL Server, MySQL, та використовував мови Delphi, С++.\r\nЯ працював як розробник проектів, займався ETL процесами міграції даних. Після фази проектування наша команда займалася створенням потоків даних, автоматичним перенесенням даних із промислових OLTP систем у сховища (DWH).\r\nДля перегляду даних використовуються звіти, і я брав активну участь у створенні звітів для вимог аудиту, аналітичних звітів та dashboards.\r\nЯ закінчив Харківський державний університет, фізичний факультет. Навчання фізики та математики допомогло мені розвинути сильні аналітичні здібності та вміння помічати деталі, концентруючись на отриманні зрозумілих результатів.\r\nПрацюючи як розробник, я підвищив рівень знання англійської до рівня B2, маю підтвердження цієї оцінки. English Benchmark test https://www.data-migration.club/r/test.jpg\r\nЯ часто буваю онлайн, і віддаю перевагу зустрічам з веб-камерою, живим спілкуванням. Дуже прошу Вас надсилати запрошення електронною поштою. Для швидкого спілкування та коротких питань телефонуйте 093 177 5176.', '', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\r-db-dba-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\db-dba-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cl\\db-dba-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\db-dba-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\db-dba-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\db-dba-ua.pdf', NULL, NULL, 0, '2024-03-20 12:49:18', '2024-10-12 21:13:19'),
(87, 'croatia-common', 'HR', 'HR', 'Хорватия общий', 'ured, rad na daljinu', '+(385) 998 511 378', 'Moje ime je Vitaly, zivim kao izbjeglica u Hrvatskoj. Molio bih vasu pomoc jer vec vise od 6 mjeseci trazim posao u Hrvatskoj.\r\nProslijedite ovu e-poruku svojim zaposlenicima.\r\nDobro govorim engleski i nekoliko sam godina radio kao vanjski zaposlenik za americke tvrtke. Nazalost, tek pocinjem uciti hrvatski, a govorim ga vrlo slabo.\r\nImajte na umu moje ukupno iskustvo od 10 godina u razvoju, implementaciji i tehnickoj podrsci u sektoru financija, osiguranja i bankarstva. Tijekom tog vremena stekao sam strucno znanje o dizajnu, razvoju i optimizaciji baze podataka. Najvise sam radio s relacijskim bazama podataka - Oracle, Microsoft SQL Server, MySQL.\r\nRadio sam kao programer baze podataka, ukljucen u procese migracije ETL podataka. Takoder je radio na automatskom prijenosu podataka iz industrijskih OLTP sustava u skladista podataka (DWH).\r\nDiplomirao sam na odsjeku za fiziku sveucilista. Proucavanje fizike i matematike pomoglo mi je razviti snazne analiticke vjestine i sposobnost uocavanja detalja dok sam se usredotocio na stvaranje razumljivih rezultata.\r\nPutem ovog linka mozete preuzeti moj zivotopis na hrvatskom jeziku.\r\n(добавить ссылки)', '', 'd:\\Мой диск\\Поиск работы\\Croatia\\r\\r-croatia-common-hr.docx', 'd:\\Мой диск\\Поиск работы\\Croatia\\cv\\croatia-common-hr.docx', 'd:\\Мой диск\\Поиск работы\\Croatia\\cl\\croatia-common-hr.docx', 'd:\\Мой диск\\Поиск работы\\Croatia\\r\\croatia-common-hr.pdf', 'd:\\Мой диск\\Поиск работы\\Croatia\\cv\\croatia-common-hr.pdf', 'd:\\Мой диск\\Поиск работы\\Croatia\\cv\\croatia-common-hr.pdf', NULL, NULL, 0, '2024-03-25 23:32:38', '2024-10-12 18:57:51'),
(91, 'Тестова посада', 'UA', 'UA', 'Тестова посада повныстю', 'Віддалена', '+(380) 93 177 51 76 Viber, Whatsapp', 'Введення 1', '', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\r-Тестова-посада-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Тестова-посада-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cl\\Тестова-посада-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\Тестова-посада-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Тестова-посада-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Тестова-посада-ua.pdf', NULL, NULL, 0, '2024-10-12 23:45:18', '2024-10-13 00:35:16'),
(92, 'Посада 2', 'UA', 'UA', 'Повністю посада 2', 'Віддал', '+(380)931 77 51 76', 'Введення 2', '', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\r-Посада-2-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Посада-2-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cl\\Посада-2-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\Посада-2-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Посада-2-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Посада-2-ua.pdf', NULL, NULL, 0, '2024-10-12 23:47:55', '2024-10-13 00:35:08'),
(93, 'Псада', 'UA', 'UA', 'Повна псада', 'Від', '+(1)585 316 19 76', 'Введення 3', '', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\r-Псада-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Псада-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cl\\Псада-ua.docx', 'd:\\Мой диск\\Поиск работы\\Ukraine\\r\\Псада-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Псада-ua.pdf', 'd:\\Мой диск\\Поиск работы\\Ukraine\\cv\\Псада-ua.pdf', NULL, NULL, 0, '2024-10-13 00:25:06', '2024-10-13 00:35:12');

-- 
-- Dumping data for table templates
--
INSERT INTO templates VALUES
(98, 'croatia-common', 'Хорватия общий', 'офис, удалённая работа', '+(385) 998 511 378', 'Меня зовут Виталий, я проживаю как беженец на территории Хорватии. Я прощу Вашей помощи, поскольку ищу работу в Хорватии более 6 месяцев.\r\nПожалуйста, перешлите это письмо специалистам по поиску персонала.\r\nЯ хорошо разговариваю по-английски и несколько лет работал как outstaff personnel для американских компаний. К сожалению, я только начинаю учить хорватский язык, и разговариваю на нем очень плохо.\r\nОбратите внимание на мой общий опыт 10 лет в разработке, внедрении и техническую поддержку в финансовой, страховой и банковской сферах. За это время я получил экспертные знания в области проектирования баз данных, разработки и оптимизации. Больше всего я работал с реляционными базами данных – Oracle, Microsoft SQL Server, MySQL.\r\nЯ работал как разработчик баз данных, занимался ETL процессами миграции данных. Также занимался автоматическим переносом данных из промышленных OLTP систем в хранилища (DWH).\r\nЯ окончил физический факультет университета. Обучение физике и математике помогло мне развить сильные аналитические способности и умение замечать детали, концентрируясь на получении понятных результатов.\r\nПо этим ссылка можно скачать моё CV на хорватском языке.\r\n(добавить ссылки)', 0, '2024-03-20 00:32:39', '2024-03-25 20:40:44');

-- 
-- Dumping data for table skills
--
INSERT INTO skills VALUES
(1, 'Data Migration', 1, '2023-11-26 11:08:45', '2024-01-06 21:14:30'),
(2, 'Database Development', 1, '2023-11-26 11:08:55', '2024-01-06 21:14:27'),
(3, 'SQL Tuning', 1, '2023-11-26 11:09:06', '2024-01-06 21:14:25'),
(4, 'Mongo database', 1, '2023-11-26 11:09:14', '2024-01-06 21:15:29'),
(5, 'Business Intelligence\r(BI) ', 8, '2023-11-26 11:09:26', '2024-02-29 02:19:20'),
(6, 'Python', 3, '2023-11-26 11:09:36', '2024-02-29 02:16:44'),
(7, 'Talend', 7, '2023-11-26 11:09:48', '2024-02-29 02:17:21'),
(8, 'Java ', 3, '2023-11-26 11:09:55', '2024-02-29 02:15:30'),
(9, 'SQL', 1, '2023-11-26 11:10:02', '2024-01-06 21:14:36'),
(10, 'ETL', 7, '2023-11-26 11:10:10', '2024-02-29 02:17:49'),
(11, 'Microsoft Azure', 6, '2023-11-26 11:10:33', '2024-02-29 02:17:43'),
(12, 'MS SQL Server', 1, '2023-11-26 11:10:42', '2024-01-06 21:14:39'),
(13, 'Oracle', 1, '2023-11-26 11:10:59', '2024-01-06 21:14:41'),
(14, 'Informatica Power Center', 7, '2023-11-26 11:11:11', '2024-02-29 02:17:54'),
(15, 'Jasper Reports', 8, '2023-11-26 11:11:27', '2024-02-29 02:18:17'),
(16, 'Talend Studio', 7, '2023-11-26 11:11:36', '2024-02-29 02:17:26'),
(17, 'MySQL', 1, '2023-11-26 11:11:45', '2024-01-06 21:14:45'),
(18, 'CentOS', 3, '2023-11-26 11:11:53', '2024-02-29 02:19:14'),
(19, 'Ubuntu', 3, '2023-11-26 11:12:02', '2024-02-29 02:18:56'),
(20, 'Bash scripts', 3, '2023-11-26 11:12:31', '2024-02-29 02:19:24'),
(21, 'DWH', 1, '2023-11-26 11:12:48', '2024-02-29 02:15:21'),
(23, 'PL/SQL', 1, '2023-11-26 11:13:14', '2024-01-06 21:15:50'),
(24, 'Performance Tuning', 1, '2023-11-26 11:13:33', '2024-02-29 02:18:29'),
(25, 'Oracle SQL Developer', 1, '2023-11-26 11:13:36', '2024-01-06 21:15:57'),
(26, 'Terasoft', 3, '2024-01-09 18:27:09', '2024-02-29 02:18:50'),
(63, 'T-SQL', 1, '2024-02-18 01:32:13', '2024-02-29 02:18:41'),
(64, 'Microsoft SQL server', 1, '2024-02-18 01:32:13', '2024-02-29 02:18:24'),
(66, 'SQL server', 1, '2024-02-18 16:35:12', '2024-02-29 02:18:34'),
(73, NULL, NULL, '2024-02-18 18:20:23', '2024-02-18 18:20:23'),
(74, NULL, NULL, '2024-02-18 18:20:34', '2024-02-18 18:20:34'),
(75, NULL, NULL, '2024-02-18 18:20:45', '2024-02-18 18:20:45'),
(76, NULL, NULL, '2024-02-18 18:20:53', '2024-02-18 18:20:53'),
(77, NULL, NULL, '2024-02-18 18:21:25', '2024-02-18 18:21:25'),
(78, NULL, NULL, '2024-02-18 18:22:00', '2024-02-18 18:22:00'),
(88, NULL, NULL, '2024-03-06 23:42:22', '2024-03-06 23:42:22'),
(89, NULL, NULL, '2024-03-18 16:08:56', '2024-03-18 16:08:56'),
(90, NULL, 0, '2024-03-18 16:09:01', '2024-03-18 16:09:01'),
(91, NULL, NULL, '2024-03-18 16:20:19', '2024-03-18 16:20:19'),
(92, NULL, NULL, '2024-03-18 16:20:34', '2024-03-18 16:20:34'),
(93, '', NULL, '2024-03-18 16:22:22', '2024-03-18 16:22:22'),
(94, 'Business Intelligence (BI)', NULL, '2024-03-18 16:25:11', '2024-03-18 16:25:11'),
(95, 'Java', NULL, '2024-03-18 16:25:11', '2024-03-18 16:25:11'),
(96, 'SQL2', NULL, '2024-03-18 16:28:17', '2024-03-18 16:28:17'),
(97, NULL, NULL, '2024-03-18 17:42:21', '2024-03-18 17:42:21'),
(98, NULL, NULL, '2024-03-18 17:42:37', '2024-03-18 17:42:37'),
(99, NULL, NULL, '2024-03-18 17:49:06', '2024-03-18 17:49:06'),
(100, NULL, NULL, '2024-03-18 17:49:20', '2024-03-18 17:49:20'),
(101, 'Test', NULL, '2024-03-18 22:24:07', '2024-03-18 22:24:07'),
(102, 'Test2', NULL, '2024-03-18 22:24:52', '2024-03-18 22:24:52'),
(103, 'Test3', NULL, '2024-03-18 22:24:52', '2024-03-18 22:24:52'),
(104, 'Test5', NULL, '2024-03-18 22:27:11', '2024-03-18 22:27:11'),
(105, 'Test6', NULL, '2024-03-18 22:29:11', '2024-03-18 22:29:11'),
(106, 'Test7', NULL, '2024-03-18 22:29:11', '2024-03-18 22:29:11'),
(107, 'Test8', NULL, '2024-03-18 22:29:11', '2024-03-18 22:29:11'),
(108, 'Test9', NULL, '2024-03-18 22:29:11', '2024-03-18 22:29:11'),
(109, 'Test Test Test Test', NULL, '2024-03-18 22:31:52', '2024-03-18 22:31:52'),
(110, 'TEMP TEMP TEMP TEMP', NULL, '2024-03-18 23:16:58', '2024-03-18 23:16:58'),
(111, 'Data Migration · Database Development · SQL Tuning · Mongo ', NULL, '2024-03-19 12:25:37', '2024-03-19 12:25:37'),
(112, 'database · Business Intelligence (BI) · Python · Talend · Java · SQL · ', NULL, '2024-03-19 12:25:37', '2024-03-19 12:25:37'),
(113, 'Microsoft Azure · MS SQL Server · Power BI · Oracle', NULL, '2024-03-19 12:30:57', '2024-03-19 12:30:57'),
(114, 'Jasper Reports · Talend Studio · Oracle · MS SQL · MySQL · CentOS · ', NULL, '2024-03-19 12:30:57', '2024-03-19 12:30:57'),
(115, 'Ubuntu · Bash scripts', NULL, '2024-03-19 12:30:57', '2024-03-19 12:30:57'),
(116, '111', NULL, '2024-03-20 00:23:58', '2024-03-20 00:23:58'),
(117, '222', NULL, '2024-03-20 00:24:46', '2024-03-20 00:24:46'),
(118, '4444444', NULL, '2024-03-20 00:40:34', '2024-03-20 00:40:34'),
(119, '11111', NULL, '2024-03-20 01:26:04', '2024-03-20 01:26:04'),
(120, '22222', NULL, '2024-03-20 01:26:04', '2024-03-20 01:26:04'),
(121, '33333', NULL, '2024-03-20 01:26:04', '2024-03-20 01:26:04'),
(122, '333', NULL, '2024-03-20 01:37:31', '2024-03-20 01:37:31'),
(123, 'MySQL · Business Intelligence (BI) · Delphi · Python · SQL', NULL, '2024-03-20 13:01:19', '2024-03-20 13:01:19'),
(124, 'Microsoft Azure · MS SQL Server · Oracle · Talend', NULL, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(125, 'Jasper Reports · Talend Studio · Oracle · MS SQL · MySQL · CentOS ', NULL, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(126, '· Ubuntu · Bash scripts', NULL, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(127, 'Informatica Power Center · Microsoft SQL server · Visual Studio · ', NULL, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(128, 'SQL Management Studio', NULL, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(129, 'Oracle · DWH · Database Development · SQL Tuning · Java · ', NULL, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(130, 'PL/SQL · Performance Tuning · Oracle SQL Developer · Informatica ', NULL, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(131, 'PowerCenter', NULL, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(132, '1111', NULL, '2024-10-09 21:01:28', '2024-10-09 21:01:28'),
(133, 'PHP', NULL, '2024-10-09 22:53:17', '2024-10-09 22:53:17'),
(134, 'MS SQL', NULL, '2024-10-10 21:47:39', '2024-10-10 21:47:39'),
(135, 'Visual Studio', NULL, '2024-10-10 21:47:39', '2024-10-10 21:47:39'),
(136, NULL, 0, '2024-10-11 10:49:03', '2024-10-11 10:49:03'),
(137, NULL, NULL, '2024-10-12 23:50:28', '2024-10-12 23:50:28'),
(138, NULL, 0, '2024-10-12 23:50:46', '2024-10-12 23:50:46'),
(139, NULL, NULL, '2024-10-12 23:50:50', '2024-10-12 23:50:50'),
(140, NULL, NULL, '2024-10-12 23:51:05', '2024-10-12 23:51:05'),
(141, 'СК1', NULL, '2024-10-13 00:25:12', '2024-10-13 00:25:12');

-- 
-- Dumping data for table experiences
--
INSERT INTO experiences VALUES
(190, NULL, NULL, 2, '4', '2020-06-01', '2024-03-05', '4', '4', '4', '', '2024-03-05 01:23:01', '2024-03-05 01:23:01'),
(349, NULL, 98, 1, 'Developer (remote) ', '2022-09-14', '2024-03-25', 'Avex ', 'Компанія розробляє веб-сайти та розробляє веб-сервіси для формування звітів. Я працював з базою даних MySQL як адміністратор і програміст. Я створив сценарії для автоматичного завантаження даних, а також створення онлайн-звітів. Для повсякденних завдань використовувалися програми Delphi та скрипти Python.', '', '', '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(350, NULL, 98, 2, 'Senior developer (віддалена) ', '2020-10-21', '2022-08-18', 'Компанія Codegenix (USA, Texas). Аутсорсер для компанії GlueUp', 'Кілька неоднорідних баз даних компанії є у виробництві (Oracle, MySQL, MongoDB). Для створення аналітичних та фінансових звітів необхідно розширити базу даних DWH та створити кілька робочих процесів для постійної передачі даних на зберігання DWH.\r\nМоя відповідальність полягала в створенні та впровадженні робочих процесів ETL. Деякі робочі процеси були створені за допомогою ETL Talend, а також використання сценаріїв Python. Після декількох місяців впровадження робочі процеси були створені та протестовані. Вони виконують усі необхідні вимоги, а керівники залишилися задоволені виконаною роботою.\r\n', 'Досвід роботи з базами даних NoSQL (MongoDB, JSON структури)', 'Рекомендаційний лист: <a>https://www.data-migration.club/cv/letter-of-recommendation.pdf</a>', '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(351, NULL, 98, 3, 'Developer (віддалена)', '2019-04-10', '2020-03-04', 'Компанія Emergn (Ukraine).  Аутсорсінг для компанії Mercer (USA)', 'Замовником проекту була страхова компанія Mercer, США. Emergn – це консалтингова компанія, що працює в Україні. Мої обов''язки в проекті - розробник баз даних. Метою проекту є щоденні робочі процеси даних із бази даних Oracle Premise до хмари Azure, бази даних Microsoft SQL. Це база даних  DWH, і в ній запускалися аналітичні звіти. Робочі процеси Talend використовувалися для передачі даних і створення звітів. На початкових етапах SQL-запити мали проблеми з низькою продуктивністю SQL, їх успішно усунули.\r\n', '', '', '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(352, NULL, 98, 4, 'Engineer (віддалена)', '2016-01-01', '2018-02-07', 'Компанія Jaspersoft (підрозділ Tibco USA)', 'Технічна підтримка додатків Jaspersoft Java - JasperReport Server і ETL Talend Studio. Сервери Jaspersoft можуть працювати з різними базами даних з джерелами даних з різних баз даних - Postgres, Oracle, MS SQL і MySQL. Я відповідаю за те, щоб допомагати користувачам встановлювати та налаштовувати Java-додатки Jaspersoft & і Talend, а також допомагати знаходити та виправляти виявлені помилки.\r\n', 'Технічна підтримка додатків Jaspersoft Java - JasperReport Server і ETL Talend Studio. Сервери Jaspersoft можуть працювати з різними базами даних з джерелами даних з різних баз даних - Postgres, Oracle, MS SQL і MySQL. Я відповідаю за те, щоб допомагати користувачам встановлювати та налаштовувати Java-додатки Jaspersoft & і Talend, а також допомагати знаходити та виправляти виявлені помилки.', '', '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(353, NULL, 98, 5, 'Developer', '2013-05-01', '2015-04-01', 'Компанія Miratech (Poland)', 'Розробка робочих процесів ETL для Lindorff (Швеція). Проект міграції даних для міграції з Oracle DWH на Microsoft SQL. Мої обов’язки полягали в аналізі найновіших вимог, створенні вихідного коду та розгортанні його на сервері Microsoft SQL. Робочі процеси даних оброблялися за допомогою Informatica Power Center.\r\n', '', '', '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(354, NULL, 98, 6, 'Support Engineer', '2011-05-03', '2012-03-08', 'Компанія Luxoft (Ukraine). Технічна підтримка для європейського банку', 'Я був розробником групи підтримки ІТ. Ми забезпечували підтримку величезного Oracle DWH в інвестиційному банку. Вихідні коди були написані на Java та PL/SQL, а робочі процеси даних використовують Informatica PC. Наша команда підтримувала щоденні прогони робочих процесів, ми також підтримували формування звітів для зовнішнього банківського аудиту. Програмне забезпечення в банках має бути без помилок і збоїв, тому весь програмний код перевірявся кілька разів, включаючи автоматичні тести QA.\r\nЯ працював у великій команді з кількох країн, і координував роботу нашої команди з іншими відділами, ми використовували ServiceNow у своїй щоденній роботі. З моєї ініціативи було запроваджено сайт Jira для покращення комунікації, я отримав заохочувальний бонус за впровадження системи JIRA.', '', '', '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(463, 82, NULL, 1, '11111111111111111', '2024-02-27', '2024-03-05', '11111111111111', '111111111111111', '111111111', '111111111111', '2024-10-12 21:13:09', '2024-10-12 21:13:09'),
(464, 86, NULL, 1, 'Developer', '2018-06-06', '2018-10-24', 'Travel USA hobbit', 'Текст\r\n<a>http://website.com</a>\r\nемейл <a>astra@gmail.com</a>\r\nАле це не точно\r\n', '', '', '2024-10-12 21:13:19', '2024-10-12 21:13:19'),
(465, 87, NULL, 1, 'Developer (remote) ', '2022-09-14', '2024-03-25', 'Avex ', 'Tvrtka izraduje web stranice i razvija web servise za generiranje izvjesca. Radio sam s MySQL bazom kao administrator i programer. Napravio sam skripte za automatsko preuzimanje podataka i generiranje mreznih izvjesca. Delphi programi i Python skripte koristeni su za svakodnevne zadatke.', '', '', '2024-10-12 21:13:26', '2024-10-12 21:13:26'),
(466, 87, NULL, 2, 'Senior developer (rad na daljinu) ', '2020-10-21', '2022-08-18', 'Tvrtka Codegenix (SAD, Texas). Outsourcer za GlueUp', 'Nekoliko heterogenih baza podataka tvrtke je u produkciji (Oracle, MySQL, MongoDB). Za izradu analitickih i financijskih izvjesca potrebno je prosiriti DWH bazu podataka i kreirati nekoliko radnih procesa za kontinuirani prijenos podataka u DWH pohranu.\r\nMoja je odgovornost bila stvoriti i implementirati ETL tijekove rada. Neki tijekovi rada stvoreni su koristenjem Talendovog ETL-a kao i koristenjem Python skripti. Nakon nekoliko mjeseci implementacije, izradeni su i testirani tijek rada. Zadovoljavaju sve potrebne uvjete, a voditelji su bili zadovoljni obavljenim poslom.', 'Iskustvo s NoSQL bazama podataka (MongoDB, JSON структури)', 'Preporuka: <a>https://www.data-migration.club/cv/letter-of-recommendation.pdf</a>', '2024-10-12 21:13:26', '2024-10-12 21:13:26'),
(467, 87, NULL, 3, 'Developer (rad na daljinu)', '2019-04-10', '2020-03-04', 'Tvrtka Emergn (Ukrajina). Outsourcing za Mercer (SAD)', 'Narucitelj projekta bila je osiguravajuca kuca Mercer, SAD. Emergn je konzultantska tvrtka koja djeluje u Ukrajini. Moje duznosti u projektu su razvoj baze podataka. Cilj projekta su dnevni tokovi podataka iz Oracle Premise baze podataka u Azure cloud, Microsoft SQL bazu podataka. Ovo je DWH baza podataka i na njoj su pokrenuta analiticka izvjesca. Za prijenos podataka i generiranje izvjesca koristeni su tijekovi rada Talenda. U pocetnim su fazama SQL upiti imali problema s niskim performansama SQL-a, koji su uspjesno eliminirani.\r\n', '', '', '2024-10-12 21:13:26', '2024-10-12 21:13:26'),
(468, 87, NULL, 4, 'Engineer (rad na daljinu)', '2016-01-01', '2018-02-07', 'Jaspersoft (odjel tvrtke Tibco USA)', 'Tehnicka podrska za Jaspersoft Java aplikacije - JasperReport Server i ETL Talend Studio. Jaspersoft posluzitelji mogu raditi s razlicitim bazama podataka s izvorima podataka iz razlicitih baza podataka - Postgres, Oracle, MS SQL i MySQL. Odgovoran sam za pomoc korisnicima pri instaliranju i konfiguriranju Jaspersoft & i Talend Java aplikacija, kao i za pomoc u pronalazenju i ispravljanju gresaka.\r\n', 'Tehnicka podrska za Jaspersoft Java aplikacije - JasperReport Server i ETL Talend Studio. Jaspersoft posluzitelji mogu raditi s razlicitim bazama podataka s izvorima podataka iz razlicitih baza podataka - Postgres, Oracle, MS SQL i MySQL. Odgovoran sam za pomoc korisnicima pri instaliranju i konfiguriranju Jaspersoft & i Talend Java aplikacija, kao i za pomoc u pronalazenju i ispravljanju gresaka.', '', '2024-10-12 21:13:26', '2024-10-12 21:13:26'),
(469, 87, NULL, 5, 'Developer', '2013-05-01', '2015-04-01', 'Компанія Miratech (Poland)', 'Розробка робочих процесів ETL для Lindorff (Швеція). Проект міграції даних для міграції з Oracle DWH на Microsoft SQL. Мої обов’язки полягали в аналізі найновіших вимог, створенні вихідного коду та розгортанні його на сервері Microsoft SQL. Робочі процеси даних оброблялися за допомогою Informatica Power Center.\r\n', '', '', '2024-10-12 21:13:26', '2024-10-12 21:13:26'),
(470, 87, NULL, 6, 'Database developer', '2011-05-03', '2012-03-08', 'Компанія Luxoft (Ukraine). Технічна підтримка для європейського банку', 'Я був розробником групи підтримки ІТ. Ми забезпечували підтримку величезного Oracle DWH в інвестиційному банку. Вихідні коди були написані на Java та PL/SQL, а робочі процеси даних використовують Informatica PC. Наша команда підтримувала щоденні прогони робочих процесів, ми також підтримували формування звітів для зовнішнього банківського аудиту. Програмне забезпечення в банках має бути без помилок і збоїв, тому весь програмний код перевірявся кілька разів, включаючи автоматичні тести QA.\r\nЯ працював у великій команді з кількох країн, і координував роботу нашої команди з іншими відділами, ми використовували ServiceNow у своїй щоденній роботі. З моєї ініціативи було запроваджено сайт Jira для покращення комунікації, я отримав заохочувальний бонус за впровадження системи JIRA.', '', '', '2024-10-12 21:13:26', '2024-10-12 21:13:26'),
(471, 93, NULL, NULL, 'НЗ1', '2024-08-07', '2024-10-02', 'КМ1', 'ОБ1', 'ВГ1', 'ІН1', '2024-10-13 00:25:12', '2024-10-13 00:25:12'),
(500, 85, NULL, 1, 'Developer & DBA', '2022-09-07', '2024-08-21', 'Avex', 'Компанія розробляє веб-сайти та веб-сервіси для формування звітів. Я працював з базою даних MySQL як адміністратор і програміст. Я створив сценарії для автоматичного завантаження даних, а також створення онлайн-звітів. Для повсякденних завдань використовувалися програми Delphi та скрипти Python.\r\n', '', '', '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(501, 85, NULL, 2, 'Senior developer', '2021-10-13', '2022-08-09', 'Codegenix (USA, Texas). Аутсорсер для компанії GlueUp', ' Кілька неоднорідних баз даних компанії є у виробництві (Oracle, MySQL, MongoDB). Для створення аналітичних та фінансових звітів необхідно розширити базу даних DWH та створити кілька робочих\r\nпроцесів для постійної передачі даних на зберігання DWH. Моя відповідальність полягала в створенні та впровадженні робочих процесів ETL. Деякі робочі процеси були створені за допомогою ETL Talend, а також використання сценаріїв Python. Після декількох місяців впровадження робочі процеси були створені та протестовані. Вони виконують усі необхідні вимоги, а керівники залишилися задоволені виконаною роботою.', 'Досвід роботи з базами даних NoSQL (MongoDB, JSON структури)', 'Рекомендаційний лист: https://www.data-migration.club/cv/letter-of-recommendation.pdf', '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(502, 85, NULL, 3, 'Developer', '2019-04-10', '2021-03-10', 'Emergn (Ukraine). Аутсорсінг для компанії Mercer (USA)', 'Замовником проекту була страхова компанія Mercer, США. Emergn – це консалтингова компанія, що працює в Україні. Мої обов''язки в проекті - розробник баз даних. Метою проекту є щоденні робочі процеси даних із бази даних Oracle Premise до хмари Azure, бази даних Microsoft SQL. Це база даних DWH, і в ній запускалися аналітичні звіти. Робочі процеси Talend використовувалися для передачі даних і створення звітів. На початкових етапах SQL-запити мали проблеми з низькою продуктивністю SQL, їх успішно усунули.', '', '', '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(503, 85, NULL, 4, 'Developer', '2018-05-08', '2019-01-23', 'Mede Analytics (Ukraine)', ' Компанія Mede-Analytics розробляє програмне забезпечення для виплат медичного страхування США. Клієнтами компанії є MEDICARE, MEDICAID, близько 200 компаній США. Документи\r\nмедичного страхування проходять повний фінансовий потік: завантаження, перевірка, зберігання в системі зберігання, відображення фінансових документів та інших звітів на веб-сайтах. Робочі процеси базуються на технології MS SQL, SSIS , OLAP Cube (MDX). Як розробник у групі впровадження я займався підтримкою бази даних. Я створив нові таблиці та процедури на основі надісланих технічних специфікацій. Для виконання завдання необхідно було вести електронне листування зі співробітниками в Америці та Європі, а також уточнювати деталі з менеджерами. Я також перевірив, чи нові структури створені на промисловий базі без помилок, і мені повідомляли про будь-які помилки, виявлені під час тестувань.', '', '', '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(504, 85, NULL, 5, 'Engineer', '2016-01-06', '2018-02-07', 'Jaspersoft (підрозділ Tibco USA)', 'Технічна підтримка додатків Jaspersoft Java - JasperReport Server і ETL Talend Studio. Сервери Jaspersoft можуть працювати з різними базами даних з джерелами даних з різних баз даних - Postgres, Oracle, MS SQL і MySQL. Я відповідаю за те, щоб допомагати користувачам встановлювати та налаштовувати Java-додатки Jaspersoft & і Talend, а також допомагати знаходити та виправляти виявлені помилки.', 'Ці програми Java працюють на всіх платформах (Windows, Unix, MacOS) і з різними базами', '', '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(505, 85, NULL, 6, 'Developer', '2013-05-08', '2015-04-08', 'Miratech (Poland)', ' Розробка робочих процесів ETL для Lindorff (Швеція). Проект міграції даних для міграції з Oracle DWH на Microsoft SQL. Мої обов’язки полягали в аналізі найновіших вимог, створенні вихідного коду та розгортанні його на сервері Microsoft SQL. Робочі процеси даних оброблялися за допомогою Informatica Power Center.', '', '', '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(506, 85, NULL, 7, 'Support Engineer', '2011-05-03', '2012-03-07', 'Luxoft (Ukraine). Технічна підтримка для європейського банку', ' Я був розробником групи підтримки ІТ. Ми забезпечували підтримку величезного Oracle DWH в інвестиційному банку. Вихідні коди були написані на Java та PL/SQL, а робочі процеси даних використовують Informatica PC. Наша команда підтримувала щоденні прогони робочих процесів, ми також підтримували формування звітів для зовнішнього банківського аудиту. Програмне забезпечення в банках має бути без помилок і збоїв, тому весь програмний код перевірявся кілька разів, включаючи автоматичні тести QA.\r\nЯ працював у великій команді з кількох країн, і координував роботу нашої команди з іншими відділами, ми використовували ServiceNow у своїй щоденній роботі. З моєї ініціативи було запроваджено сайт Jira для покращення комунікації, я отримав заохочувальний бонус за впровадження системи JIRA.', '', '', '2024-10-13 00:57:51', '2024-10-13 00:57:51');
...
--
INSERT INTO words_translations VALUES
(1, 'Обязанности', 'Обов`язки', 'Responsibilities', 'Odgovornosti', 'Obowiązki', 'Verantwortlichkeiten'),
(2, 'Преимущества', 'Переваги', 'Benefits', 'Prednosti', 'Korzyści', 'Vorteile'),
(3, 'Должность', 'Посада', 'Position', 'Položaj', 'Pozycja', 'Position'),
(4, 'Сопроводительное письмо', 'Супровідний лист', 'Cover letter ', 'Popratno pismo', 'List motywacyjny', 'Motivationsschreiben'),
(5, 'Опыт работы', 'Досвід роботи', 'Work experience', 'Radno iskustvo', 'Doświadczenie zawodowe', 'Arbeitserfahrung'),
(6, 'Навыки', 'Навички', 'Skills', 'Vještine', 'Umiejętności', 'Fähigkeiten'),
(7, 'Причина увольнения', 'Причина звільнення', 'Reason for dismissal', 'Razlog otkaza', 'Powód zwolnienia', 'Grund für die Entlassung'),
(8, 'Рекомендательное письмо', 'Рекомендаційний лист', 'Letter of recommendation', 'Pismo preporuke', 'List polecający', 'Empfehlungsschreiben');

-- 
-- Dumping data for table template_footers
--
INSERT INTO template_footers VALUES
(312, 98, 'Мови', 'Поглиблений рівень англійської мови. Носій української та російської мов. \r\nСередній рівень німецької, польської мов.', 1, '2024-10-09 21:29:22', '2024-10-09 21:29:22'),
(313, 98, 'Курси навчання', 'Oracle 11 DBA (database administrator), Informatica Power Center', 3, '2024-10-09 21:29:22', '2024-10-09 21:29:22');

-- Table jobsearchgit.tags does not contain any data (it is empty)

-- 
-- Dumping data for table skill_show_lists
--
INSERT INTO skill_show_lists VALUES
(1, 501, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(2, 500, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(2, 501, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(2, 506, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(3, 501, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(3, 506, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(4, 501, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(7, 502, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(10, 350, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(11, 502, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(11, 503, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(12, 502, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(12, 503, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(13, 502, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(13, 504, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(13, 506, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(14, 503, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(14, 505, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(15, 504, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(16, 504, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(17, 500, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(21, 506, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(64, 505, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(94, 500, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(111, 350, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(112, 350, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(117, 463, '2024-10-12 21:13:09', '2024-10-12 21:13:09'),
(122, 463, '2024-10-12 21:13:09', '2024-10-12 21:13:09'),
(123, 349, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(123, 464, '2024-10-12 21:13:19', '2024-10-12 21:13:19'),
(124, 351, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(125, 352, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(126, 352, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(127, 353, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(128, 353, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(128, 503, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(128, 505, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(129, 354, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(130, 354, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(131, 354, '2024-03-25 20:52:19', '2024-03-25 20:52:19'),
(132, 463, '2024-10-12 21:13:09', '2024-10-12 21:13:09'),
(133, 500, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(134, 504, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(135, 505, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(141, 471, '2024-10-13 00:25:12', '2024-10-13 00:25:12');

-- 
-- Dumping data for table resume_footers
--
INSERT INTO resume_footers VALUES
(307, 82, 'Technische Fahigkeiten und Technologie', '1', 1, 1, '2024-10-12 21:13:09', '2024-10-12 21:13:09'),
(308, 82, 'Technische 2 Fahigkeiten und Technologie', '1', 2, 0, '2024-10-12 21:13:09', '2024-10-12 21:13:09'),
(309, 82, 'Technische 3 Fahigkeiten und Technologie', '3', 3, 0, '2024-10-12 21:13:09', '2024-10-12 21:13:09'),
(310, 82, 'Technische 4 Fahigkeiten und Technologie', '4', 4, 1, '2024-10-12 21:13:09', '2024-10-12 21:13:09'),
(311, 86, 'Технічні навички та технології', 'DBA & Development для баз Microsoft SQL, Oracle - 10 років \r\nMS Azure – 3 роки \r\nSQL запроси - 10 років \r\nReports and dashboards – 5 років \r\nLinux Systems (Red hat & Ubuntu) - 6 років \r\nPHP – 3 роки \r\nPython – 1 рік', 1, 1, '2024-10-12 21:13:19', '2024-10-12 21:13:19'),
(312, 86, 'Мови', 'Поглиблений рівень англійської мови. Носій української та російської мов. \r\nСередній рівень німецької, польської мов', 2, 0, '2024-10-12 21:13:19', '2024-10-12 21:13:19'),
(313, 86, 'Курси навчання', 'Oracle 11 DBA (database administrator), Informatica Power Center', 3, 0, '2024-10-12 21:13:19', '2024-10-12 21:13:19'),
(314, 86, 'Засоби зв’язку зі мною', 'Я часто буваю онлайн, і віддаю перевагу зустрічам з веб-камерою, живим спілкуванням.\r\nДуже прошу Вас надсилати запрошення електронною поштою. Для швидкого спілкування та коротких питань телефонуйте +38093 177 5176.', 4, 1, '2024-10-12 21:13:19', '2024-10-12 21:13:19'),
(315, 87, 'Jezici', 'Napredna razina engleskog jezika. Izvorni govornik ukrajinskog i ruskog jezika.\r\nSrednji nivo njemackog, poljskog.', 1, 0, '2024-10-12 21:13:26', '2024-10-12 21:13:26'),
(316, 87, 'Tecajevi obuke', 'Oracle 11 DBA (database administrator), Informatica Power Center', 3, 0, '2024-10-12 21:13:26', '2024-10-12 21:13:26'),
(317, 91, 'Нав 1', 'Текст 1', 1, 1, '2024-10-12 23:45:18', '2024-10-12 23:45:18'),
(318, 91, 'Нав 2', 'Текст 2', 2, 1, '2024-10-12 23:45:18', '2024-10-12 23:45:18'),
(319, 91, 'Нав 3', 'Текст 3', 3, 0, '2024-10-12 23:45:18', '2024-10-12 23:45:18'),
(320, 91, 'Нав 4', 'Текст 4', 4, 0, '2024-10-12 23:45:18', '2024-10-12 23:45:18'),
(321, 92, 'Нава 1', 'Текст нава 1', 1, 1, '2024-10-12 23:47:55', '2024-10-12 23:47:55'),
(322, 92, 'Нава 2', 'Текст нава 2', 2, 0, '2024-10-12 23:47:55', '2024-10-12 23:47:55'),
(323, 92, 'Нава 3', 'Текст нава 3', 3, 0, '2024-10-12 23:47:55', '2024-10-12 23:47:55'),
(324, 92, 'Нава 4', 'Текст нава 4', 4, 1, '2024-10-12 23:47:55', '2024-10-12 23:47:55'),
(325, 93, 'Н1', 'Р1', 1, 1, '2024-10-13 00:25:06', '2024-10-13 00:25:06'),
(326, 93, 'Н2', 'Р2', 2, 1, '2024-10-13 00:25:06', '2024-10-13 00:25:06'),
(327, 93, 'Н3', 'Р3', 3, 0, '2024-10-13 00:25:06', '2024-10-13 00:25:06'),
(328, 93, 'Н4', 'Р4', 4, 0, '2024-10-13 00:25:06', '2024-10-13 00:25:06'),
(345, 85, 'Технічні навички та технології', 'Linux Systems (Red hat & Ubuntu) - 6 років\r\nMS Azure – 3 роки\r\nDevelopment for Oracle databases, Microsoft SQL server - 7 років\r\nSQL queries - 10 років\r\nReports and dashboards – 5 років\r\nPHP – 2 роки\r\nJava – 2 роки\r\nPython – 1 рік', 1, 1, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(346, 85, 'Мови', 'Поглиблений рівень англійської мови. Носій української та російської мов.', 2, 1, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(347, 85, 'Курси навчання', 'Oracle 11 DBA (database administrator), Informatica Power Center', 3, 1, '2024-10-13 00:57:51', '2024-10-13 00:57:51'),
(348, 85, 'Засоби зв’язку зі мною', 'Я часто буваю онлайн, і віддаю перевагу зустрічам з веб-камерою, живим спілкуванням.\r\nДуже прошу Вас надсилати запрошення електронною поштою. Для швидкого спілкування та коротких питань телефонуйте +38093 177 5176.', 4, 0, '2024-10-13 00:57:51', '2024-10-13 00:57:51');
...
(2, 'english_test', 'test.jpg'),
(3, 'website', 'https://data-migration.club/r/'),
(4, 'fullname', 'Makhaiev Vitalii'),
(5, 'email', 'vitaly.makhaev@gmail.com'),
(6, 'en_telephone', '+(1)585 316 19 76'),
(7, 'hr_telephone', '+(385) 998 511 378'),
(8, 'ua_telephone', '+(380) 93 177 51 76 Viber, Whatsapp'),
(9, 'us_telephone', '+(1)585 316 19 76'),
(10, 'main_folder', 'd:\\Мой диск\\Поиск работы'),
(11, 'main_folder_VESTA', 'd:\\Мой диск\\Поиск работы'),
(12, 'main_folder_LAPTOP-PTERIK', 'h:\\Мой диск\\Поиск работы'),
(13, 'de_telephone', '+(385) 998 511 378'),
(14, 'po_telephone', '+(385) 998 511 378'),
(15, 'ru_telephone', '+(380)931 77 51 76');

-- Table jobsearchgit.jobsites_posts does not contain any data (it is empty)

-- Table jobsearchgit.emails does not contain any data (it is empty)

-- Table jobsearchgit.companies does not contain any data (it is empty)

-- Table jobsearchgit.activity_tasks does not contain any data (it is empty)
